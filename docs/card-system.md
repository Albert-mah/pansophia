...
- **交互复习题**(`lab` 字段):讲解页里的交互演示(`.lab` 块)配的理解检验题。
  `lab` = 该交互 `.lab-title` 的标题原文(含 emoji,前端按去空格包含匹配),
  `kp` = **页面 catalog id**(交互题是课文复习题,跟页面走,不跟考点标题走),
  `point` 可省。前端把它内嵌渲染在对应交互块正下方(玩完顺手答),不进小节导航习题圆点;
  普通刷题流里照常出现,所以 stem 必须自含交互场景描述(不看页面也能作答)。
  每个交互 1 题为主,信息量大的交互至多 2 题。
...
- **基本分模型(2026-07-03)**:每个知识点小点「学到」(首次点懂了)+1(`pt:<kp>#<idx>`);
  每道题「答对」+2~5(`q:<id>`);整个考点「掌握」+knowledgeValue(`kp:<ref>`)。
  三层全部 awardOnce 一生一次,学到 → 答对 → 掌握逐层累进。
- **课外习题约定**:超纲/兴趣题入库时写 `"scope": "extra"`。课内练习、混合练、模拟卷、
  复习抽题、小节导航圆点、配题达标统计**全部排除** extra;答对照样 awardOnce 给分,
  但**不影响考点的掌握门禁与正确率**(前端做题记录时课外题不挂 kp 统计)。
