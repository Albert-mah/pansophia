...
def init_db():
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            cur.execute("ALTER TABLE questions ADD COLUMN IF NOT EXISTS lab text;")     # 交互复习题:所属课文交互(.lab-title 原文),前端内嵌到交互块下方
            # 答题记录:驱动错题本 / 掌握 / 自适应选题
            cur.execute("""
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def api_questions_get(self, q):
        ...
        sql = "SELECT id,kp,subject,scope,edition,type,difficulty,variant_of,stem,options,answer,explain,source,point,audio,lab FROM questions"
    ...
    def api_questions_post(self):
        ...
        try:
            ...
            with conn, conn.cursor() as cur:
                for it in items[:500]:
                    cur.execute("""INSERT INTO questions(kp,subject,scope,edition,type,difficulty,variant_of,stem,options,answer,explain,source,point,audio,lab)
                        ...
                        (it.get("kp"), it.get("subject"), it.get("scope"), it.get("edition"),
                         ...
                         (str(it.get("lab"))[:120] if it.get("lab") else None)))
