...
5. **图表 / 视觉元素尽量多(纯 HTML+CSS/内联 SVG,禁外部图片和图表库)**:每页**至少 1 个图表型元素**,能上 2-3 个更好。可选形式:
   - **CSS 横条 / 柱状图**(div 宽高按数据比例,标数值)—— 对比数量级、占比;
   - **流程图 / 步骤链**(flex 一排圆角块 + → 箭头)—— 讲顺序、因果;
   - **对比双栏卡**(好 vs 坏、before vs after 两列)—— 讲取舍;
   - **时间轴 / 阶梯**(border-left 竖线 + 节点)—— 讲演进、层级;
   - **内联 SVG 示意图**(几何、坐标、结构)—— 讲空间关系;
   - **语义化表格**(`.page` 里已有 table 样式)—— 多维属性对照。
   全部套页面前缀类,颜色用 CSS 变量,数据直接写死在标记里(不依赖 JS 也能看)。
6. **层次分明,一眼扫出骨架**:正文按「TL;DR → 3-5 个编号小节(h2 带 01/02/03 序号)→ 试一试 → 关联」组织;每小节内部固定节奏 = 一段正文 + 一个视觉元素(box/图表/交互/折叠)交替,**不要连续 3 段以上纯文字**;`.box` 四种色(key/tip/warn/example)交替用,别整页同色。
...
- 每页 ≥2 个交互、≥1 个图表型元素(CSS 图/SVG/对比卡/流程链)、≥1 个可点开折叠、有 TL;DR 与试一试、编号小节。

## 六、可参考的现有好样例
- **金标准:`subjects/design/design-ucd-cognitive-load.html`** —— 按本 spec 全部六条要求重写过(TL;DR、编号小节、CSS 图表、对比卡、多交互、折叠深入),照它的密度和层次做。
