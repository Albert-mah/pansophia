  ...
  function KpSummaryPanel(p) {
    ...
    return html`<div>
      ...
      <div style="margin-top:14px;border:1px solid #EEE3CF;border-radius:14px;padding:16px 18px;background:#fff;">
        ...
        <textarea class="pan-note-input" placeholder="记下你的理解、易错点…(回车保存,只挂在这一节下;在讲解里选中文字点 ⭐ 也会收进这里)" onKeyDown=${function (e) { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); addNote(e.target.value); e.target.value = ""; } }}></textarea>
        ${myNotes.map(function (n, i) { return html`<div key=${i} style=${"display:flex;gap:8px;align-items:flex-start;border-radius:10px;padding:10px 12px;margin-top:9px;" + (n.starred ? "background:#FBF4E6;border-left:3px solid #C8852E;" : "background:#FBF6EC;")}><div style="flex:1;min-width:0;">${n.starred ? html`<div style="font-size:10.5px;font-weight:700;color:#C8852E;margin-bottom:3px;">⭐ 讲解摘录</div>` : null}<div style="font-size:13px;line-height:1.65;color:#3a3023;white-space:pre-wrap;">${n.body || n.title}</div></div><span onClick=${function () { delNote(n); }} title="删除" style="cursor:pointer;color:#cbb9a0;font-size:14px;">✕</span></div>`; })}
      </div>
      <div style="margin-top:14px;border:1px solid #EEE3CF;border-radius:14px;padding:16px 18px;background:#fff;">
