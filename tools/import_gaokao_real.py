...
REPL = [
    ...
    (r"\\cdot", "·"), (r"\\triangle", "△"), (r"\\overrightarrow\{([^{}]*)\}", r"\1⃗"), (r"\\vec\{([^{}]*)\}", r"\1⃗"), (r"\\times", "×"), (r"\\div", "÷"), (r"\\pm", "±"),
    (r"\\%", "%"), (r"\\\$", "$"), (r"\\&", "&"), (r"\\_", "_"), (r"\\#", "#"),
...
KP_RULES = [
    ...
    ("复数的概念与几何意义", ["共轭", "纯虚数", "复平面", "虚部", "实部"]),
    ("复数的四则运算", ["复数", "虚数"]),
    ("平面向量的数量积", ["数量积", "投影"]),
    ("向量的夹角、模与垂直平行的判定", ["垂直", "夹角"]),
    ("平面向量基本定理与坐标表示", ["基底", "坐标表示"]),
    ("平面向量的概念与线性运算", ["向量"]),
    ("等差数列及其前n项和", ["等差"]),
    ("等比数列及其前n项和", ["等比"]),
    ("由递推关系求通项公式", ["递推", "aₙ₊₁", "a_(n+1)"]),
    ("数列的概念与通项公式", ["数列", "通项"]),
    ("正弦定理与余弦定理", ["解三角形", "正弦定理", "余弦定理", "△", "三角形ABC", "三角形 ABC"]),
    ("三角函数的图象与性质", ["sin", "cos", "tan", "三角函数"]),
]
# 出现这些词直接跳过(章节还没做,或题型不适合)
SKIP_WORDS = ["抛物线", "双曲线", "椭圆", "圆锥曲线",
...
def main():
    ...
    for f in ("gaokao_math1_mcq.json", "gaokao_math2_mcq.json"):
        for q in parse_file(os.path.join(HERE, "sources", f)):
            ...
            text = unlatex(raw)
            if "\\" in text or "＼" in text or "mathrm" in text: skipped_latex += 1; continue   # 转不干净的放弃(含全角反斜杠/mathrm 残留)
